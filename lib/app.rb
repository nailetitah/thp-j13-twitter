...
def bonjour_monde(client)
  client.update("Oui, #bonjour_monde")
end


def bonjour_journalists(client)
...
arr.sample(5).each { |account| client.update("Hello, #{account} ! Il semblerait que mon bot fonctionne... Merci bien ! #bonjour_monde") }
end

def favorite(client)
client.search("#bonjour_monde", result_type: "recent").take(1).each { |tweet|
...
def follow(client)
user_bonjour_monde = []
client.search("#bonjour_monde -rt", result_type: "recent").take(5).collect do |tweet|
  user_bonjour_monde << tweet.user.screen_name
end
user_bonjour_monde.uniq.each { |user| client.follow(user) if user != "N_Titah" }
end
